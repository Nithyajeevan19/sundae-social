...
function Index() {
  ...
  const handleEmotion = (e: Emotion) => {
    setEmotion(e);
    if (e === "improve") setStage("feedback");
    else setStage("follow");
    fire();
  };
        ...
        <AnimatePresence mode="wait">
          ...
          {stage === "follow" && (
            <motion.div key="follow">
              <ReviewFlow onDone={() => { fire(); setStage("story"); }} />
              <button
                onClick={() => { setStage("pick"); setEmotion(null); }}
          ...
          {stage === "feedback" && (
            <motion.div key="feedback">
              <FeedbackForm onSubmit={() => { fire(); setStage("story"); }} />
              <button
                onClick={() => { setStage("pick"); setEmotion(null); }}
                className="mt-4 w-full text-center text-xs text-muted-foreground underline"
              >
          ...
          {stage === "story" && (
            <motion.div key="story" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <div className="rounded-3xl bg-gradient-blush p-5 text-center shadow-pop">
                ...
                <h3 className="mt-1 font-display text-xl text-chocolate">Thank you, sweet soul!</h3>
